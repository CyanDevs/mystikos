...
pipeline {
    ...
    stages {
        ...
        stage('Stash changes') {
            steps {
                sh "tar -zcvf CC-${BUILD_NUMBER}.tar.gz ${WORKSPACE}"
                azureUpload(
                    storageType: 'container',
                    containerName: 'mystikos-build-resources',
                    uploadZips: true,
                    filesPath: "CC-BUILD-${BUILD_NUMBER}.tar.gz",
        ...
        stage('Parallel tests') {
            parallel {
                stage('General Tests and Code Coverage') {
                    stages {
                        stage('Run all tests') {
                            steps {
                        ...
                        stage('Unstash') {
                            steps {
                                azureDownload(
                                    ...
                                    containerName: 'mystikos-build-resources',
                                    includeFilesPattern: "CC-BUILD-${BUILD_NUMBER}.tar.gz",
                                    storageCredentialId: 'mystikosreleaseblobcontainer'
                                )
                                sh "tar xvzf CC-BUILD-${BUILD_NUMBER}.tar.gz"
                            }
                        }
